...
export class MockProvider implements AIProvider {
  ...
  async qualifyLead(input: NormalizedLead): Promise<AIQualificationOutput> {
    const text = `${input.content.message ?? ""} ${input.content.transcript ?? ""}`.toLowerCase();

    if (containsAny(text, ["spam", "unsubscribe", "crypto giveaway"])) {
      return this.buildLowPriorityOutput(input);
    }

    if (containsAny(text, ["book", "booking", "appointment", "consultation", "quote", "call me back"])) {
  ...
  private buildHighPriorityOutput(input: NormalizedLead): AIQualificationOutput {
    return {
      ...
      priority: "High",
      lead_score: 86,
      service_requested: input.content.transcript ? "Service quote" : "Appointment booking automation",
  ...
  private buildMediumPriorityOutput(input: NormalizedLead): AIQualificationOutput {
    return {
  ...
  private buildLowPriorityOutput(input: NormalizedLead): AIQualificationOutput {
    return {
      lead_summary: "The message does not show a clear service request or buying intent.",
      lead_type: "Other",
      intent: "Other",
      priority: "Low",
      lead_score: 22,
